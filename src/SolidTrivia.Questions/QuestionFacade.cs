...
namespace SolidTrivia.Questions
{
    public class QuestionFacade : IQuestionFacade
    {
        ...
        public NewBoard GetBoard(Guid boardId)
        {
        ...
        public void RenameBoard(Guid boardId, string newBoardName)
        {
        ...
        public NewCategory GetCategory(Guid categoryId) => categoryStore.GetById(categoryId);

        public void RenameCategory(Guid categoryId, string newCategoryName)
        {
        ...
        #region Comment
        public IEnumerable<NewComment> ListComments(Guid questionId) => commentStore.ListCommentsOfQuestion(questionId);
        #endregion
        ...
        public NewQuestion GetQuestion(Guid questionId)
        {
        ...
        public bool TagExists(Guid tagId) => tagStore.Exists(tagId);

        public NewTag GetTag(Guid tagId)
        {
        ...
        public void RenameTag(Guid tagId, string newTagName)
        {
        ...
        #region Vote
        public void UpVote(string userId, Guid questionId) => voteStore.UpVote(userId, questionId);
        public void DownVote(string userId, Guid questionId) => voteStore.DownVote(userId, questionId);
        public (int, int) GetVotes(Guid questionId) => voteStore.GetVotes(questionId);
        #endregion
        ...
        public void AddCategoryToBoard(Guid boardId, Guid categoryId)
        {
            if (!boardStore.Exists(boardId)) throw new ArgumentException(nameof(boardId), $"Board with id '{boardId}' does not exist");
            if (!categoryStore.Exists(categoryId)) throw new ArgumentException(nameof(categoryId), $"Category with id '{categoryId}' does not exist");
            categoryStore.AddToBoard(boardId, categoryId);
        }
        public void DeleteCategoryOfBoard(Guid boardId, Guid categoryId)
        {
            if (!boardStore.Exists(boardId)) throw new ArgumentException(nameof(boardId), $"Board with id '{boardId}' does not exist");
            if (!categoryStore.Exists(categoryId)) throw new ArgumentException(nameof(categoryId), $"Category with id '{categoryId}' does not exist");
        ...
        public IEnumerable<NewQuestion> ListQuestionsOfCategory(Guid categoryId) => questionStore.ListQuestionsOfCategory(categoryId);

        //comments and questions
        public void ReplyToQuestion(Guid questionId, string comment) => commentStore.ReplyToQuestion(questionId, comment);

        public void ReplyToComment(Guid questionId, Guid commentId, string reply) => commentStore.ReplyToComment(questionId, commentId, reply);

        public IEnumerable<NewComment> ListRepliesOfComment(Guid questionId, Guid commentId) => commentStore.ListRepliesOfComment(questionId, commentId);


        public void RemoveCategoryFromBoard(Guid boardId, Guid categoryId)
        {
            if (!boardStore.Exists(boardId)) throw new ArgumentException(nameof(boardId), $"Board with id '{boardId}' does not exist");
            if (!categoryStore.Exists(categoryId)) throw new ArgumentException(nameof(categoryId), $"Category with id '{categoryId}' does not exist");
            categoryStore.RemoveFromBoard(boardId, categoryId);
        ...
        public IEnumerable<NewCategory> ListAvailableCategories(Guid boardId)
        {
            if (!boardStore.Exists(boardId)) throw new ArgumentException(nameof(boardId), $"Board with id '{boardId}' does not exist");
            return categoryStore.ListAvailable(boardId);
        }

        public void EditQuestionContent(Guid questionId, string userInputMarkdown)
        {
